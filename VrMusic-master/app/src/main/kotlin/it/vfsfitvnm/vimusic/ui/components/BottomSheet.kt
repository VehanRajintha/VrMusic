...
fun BottomSheet(
    ...
    Box(
        modifier = modifier
            .offset {
                val x = (state.expandedBound - state.value)
                    .roundToPx()
                    .coerceAtLeast(0)
                IntOffset(x = x, y = 0)
            }
    ...
    val preUpPostDownNestedScrollConnection
        get() =  object : NestedScrollConnection {
            ...
            override fun onPreScroll(available: Offset, source: NestedScrollSource): Offset {
                if (isExpanded && available.x > 0) {
                    isLeftReached = false
                }

                return if (isLeftReached && available.x > 0 && source == NestedScrollSource.Drag) {
                    dispatchRawDelta(available.x)
                    available
                } else {
                    Offset.Zero
                }
            ...
            override fun onPostScroll(
                ...
                if (!isLeftReached) {
                    isLeftReached = consumed.x == 0f && available.x < 0
                }

                return if (isLeftReached && source == NestedScrollSource.Drag) {
                    dispatchRawDelta(available.x)
                    available
                } else {
                    Offset.Zero
